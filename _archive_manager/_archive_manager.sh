...
am_main(){
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --list) action=list; shift; archive="${1:-}" ;;
      --extract) action=extract; shift; archive="${1:-}" ;;
      --dest) shift; dest="${1:-.}" ;;
      --create) action=create; shift; out="${1:-}" ;;
      -h|--help) am_usage; return 0 ;;
      *) paths+=("$1") ;;
    esac
    shift
  done
  ...
  case "$action" in
    list)
      ...
      case "$archive" in
        *.zip) uk_has_cmd unzip || { uk_error 'unzip is required for zip archives.'; return 1; }; unzip -l "$archive" ;;
        *) tar -tf "$archive" ;;
      esac
      ;;
    extract)
      ...
      if bad_paths="$(am_check_archive_paths "$archive")" && [[ -z "$bad_paths" ]]; then
        mkdir -p "$dest"
        case "$archive" in
          *.zip) uk_has_cmd unzip || { uk_error 'unzip is required for zip archives.'; return 1; }; unzip "$archive" -d "$dest" ;;
      ...
      ;;
    create)
      [[ -n "$out" && ${#paths[@]} -gt 0 ]] || { am_usage; return 1; }
      case "$out" in
        *.zip) uk_has_cmd zip || { uk_error 'zip is required to create zip archives.'; return 1; }; zip -r "$out" "${paths[@]}" ;;
        *) tar -czf "$out" "${paths[@]}" ;;
      esac
