...
#   1. every registry tool has its directory + script + README on disk
...
uk_doctor() {
  ...
  local problems=0 warnings=0 checked=0 fixed=0
  ...
  local rec key action rest path dispatch_body readme name desc
  ...
  for rec in "${UK_REGISTRY[@]}"; do
    ...
    # (c) README exists?
    readme="${path%/*}/_${key}_README.md"
    if [[ ! -f "$readme" ]]; then
      if [[ "$fix" -eq 1 ]]; then
        name='' desc=''
        IFS='|' read -r _ _ _ _ name desc _ <<<"$rec"
        printf '# _%s\n\n%s — %s\n\n> Stub generated by `main.sh doctor --fix`. Replace with real documentation.\n' \
          "$key" "${name:-$key}" "${desc:-}" >"$readme"
        printf '  %s %-20s created README stub: %s\n' "$ok" "$key" "${readme#"$UK_ROOT_DIR"/}"
        fixed=$((fixed + 1))
      else
        printf '  %s %-20s missing README: %s\n' "$wrn" "$key" "${readme#"$UK_ROOT_DIR"/}"
        warnings=$((warnings + 1))
      fi
    fi
    # (d) dispatch case exists for the action?
  ...
  for d in "$UK_ROOT_DIR"/modules/_*/; do
    [[ -d "$d" ]] || continue
    dkey="$(basename "$d")"
    dkey="${dkey#_}"
    found=0
    ...
    if [[ "$found" -eq 0 ]]; then
      ...
      if [[ "$fix" -eq 1 ]]; then
        printf '    %snot auto-deleted — review, then run:%s git rm -r "modules/_%s"\n' \
          "$UK_C_DIM" "$UK_C_RESET" "$dkey"
      fi
      warnings=$((warnings + 1))
      orphans=$((orphans + 1))
    fi
