...
wt_main(){ local loc='' units='metric'; while [[ $# -gt 0 ]]; do case "$1" in --units) shift; units="${1:-metric}";; -h|--help) wt_usage; return 0;; *) loc="$1";; esac; shift; done; local cache="$(uk_state_dir)/weather_last.txt" u=m; [[ "$units" == imperial ]] && u=u; uk_has_cmd curl || { uk_warn 'curl unavailable; cache only.'; [[ -f "$cache" ]] && cat "$cache"; return 0; }; curl -fsS --max-time 8 "https://wttr.in/${loc}?format=3&${u}" | tee "$cache" || { uk_warn 'Weather lookup failed.'; [[ -f "$cache" ]] && cat "$cache"; }; }
