...
**v5.4.0** - current — `doctor` gained an opt-in `--fix` flag: creates missing `modules/_<tool>/_<tool>_README.md` stubs from registry metadata, and flags orphan tool directories with a suggested `git rm -r` command (never auto-deletes). Doctor now also checks each registry tool for its README, and the orphan-directory scan was repaired (it globbed the repo root instead of `modules/` and never matched anything).
