...
t_todo_done_empty() {
  local tmp out rc
  tmp="$(mktemp -d)" || return 1
  out="$(XDG_DATA_HOME="$tmp/data" bash -c 'source "'$ROOT'/lib/uk_common.sh"; source "'$ROOT'/_todo_manager/_todo_manager.sh"; td_main --done ""' 2>&1)"
  rc=$?
  rm -rf "$tmp"
...
t_todo_done_valid() {
  local tmp f
  tmp="$(mktemp -d)" || return 1
  XDG_DATA_HOME="$tmp/data" bash -c 'source "'$ROOT'/lib/uk_common.sh"; source "'$ROOT'/_todo_manager/_todo_manager.sh"; td_main --add "write tests" --tag qa; td_main --done 1' >/dev/null 2>&1 || {
    rm -rf "$tmp"
    return 1
  }
