...
register_managers() {
  ...
  register_manager "nix" "system" "has_cmd nix-env || has_cmd nix" "if has_cmd nix-channel; then nix-channel --update || exit 1; fi; if has_cmd nix-env; then nix-env -u '*' || exit 1; fi; if has_cmd nix; then nix profile upgrade '.*' || exit 1; fi" "(has_cmd nix-env && nix-env -q --outdated 2>/dev/null || true) | sed -n '1,8p'" "Nix pkgs/profiles"
  ...
  register_manager "uv" "language" "has_cmd uv" "uv self update && uv tool upgrade --all" "uv tool list 2>/dev/null | sed -n '1,12p'" "Astral uv tools"
  ...
  register_manager "yarn" "language" "has_cmd yarn" "yarn global upgrade || yarn set version stable" "yarn global outdated 2>/dev/null | sed -n '1,12p'" "Yarn global pkgs"
  ...
  register_manager "cargo" "language" "has_cmd cargo" "if has_cmd cargo-install-update; then cargo install-update -a; elif has_cmd rustup; then rustup update; else printf 'No Cargo update backend found\n' >&2; false; fi" "(has_cmd cargo-install-update && cargo install-update -l 2>/dev/null || cargo install --list 2>/dev/null) | sed -n '1,16p'" "Cargo binaries"
